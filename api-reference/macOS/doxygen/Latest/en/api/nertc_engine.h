...
public:
     ...
     * - 该接口需要在调用 `leaveChannel`、并收到本端离开房间的回调后调用。或收到 `onDisconnect`
     * 回调、重连失败时调用此接口销毁实例，并释放资源。
     * - 调用 `release` 方法后，您将无法再使用 SDK
     * 的其它方法和回调。如需再次使用实时音视频通话功能，您必须重新创建一个新的 NERtc 实例。
     * @note
     * - 该方法为同步调用，需要等待 NERtcEngine
     * 实例资源释放后才能执行其他操作，建议在子线程中调用该方法，避免主线程阻塞。此外，请勿在 SDK 的回调中调用
     * `release`，否则由于 SDK 要等待回调返回才能回收相关的对象资源，会造成死锁。SDK
     * 会自动检测这种死锁并转为异步调用，但是检测本身会消耗额外的时间。
     * - 如果需要重新使用 IRtcEngine，`release` 后需要调用 `destroyNERtcEngine` 销毁引擎，等待执行结束后才能再次调用
     * `createNERtcEngine`。
