...
struct NERtcLiveStreamImageInfo {
    ...
    NERtcLiveStreamImageInfo() : x(0), y(0), width(0), height(0) { 
...
struct NERtcLiveStreamLayout {
    ...
    NERtcLiveStreamLayout() : width(0), height(0), background_color(0), user_count(0), users(NULL), bg_image(NULL) {}
};
...
typedef enum
{
     ...
     * @if Chinese
     * 正在加入房间。
     * <br>该状态表示 SDK 处于调用 joinChannel 接口之后，正在建立房间连接的阶段。如果加入房间成功 App 会收到 onConnectionStateChange 回调，当前状态变为 kNERtcConnectionStateConnected。
     * @endif
     */
     ...
     * @if Chinese
     * 加入房间成功。
     * <br>该状态表示用户已经加入房间，如果因网络断开或切换而导致 SDK 与房间的连接中断，SDK 会自动重连，此时 App 会收到 onConnectionStateChange 回调 ，当前状态变为 kNERtcConnectionStateReconnecting。
     * @endif
     */
     ...
     * @if Chinese
     * 正在尝试重新加入房间。
     * <br>该状态表示 SDK 之前曾加入过房间，但因为网络原因中断了，此时 SDK 会自动尝试重新加入房间。如果重连还是没能加入房间会触发 onConnectionStateChange 回调， 当前状态变为 kNERtcConnectionStateFailed，SDK 停止尝试重连。
     * @endif
     */
...
typedef enum {
     ...
     * @if Chinese
     * 屏幕共享的目标窗口被覆盖。
     * @note 在 Windows 平台中，某些共享目标窗口可能会因处于图层下层被屏蔽，此窗口图像可能会黑屏并触发此回调。建议应用层在触发此回调时提醒用户将待分享的窗口置于最上层。
     * @endif
     */
     ...
     * 屏幕共享异常结束。
     * @since V4.6.0
     * @note 由于屏幕共享的目标窗口被关闭或者被分享的进程崩溃等原因导致目标窗口无效时，会触发此回调。用户需要在这个事件的响应函数中调用停止屏幕共享接口结束共享。
     * @endif
    */
...
struct NERtcVideoCanvas
{
    ...
    NERtcVideoCanvas() : user_data(NULL), window(NULL), scaling_mode(kNERtcVideoScaleFit), mirror_mode(kNERtcVideoMirrorModeDisabled) {}
};
...
public:
     ...
     * @deprecated This API is about to be deprecated. Use \ref nertc::INERtcAudioFrameObserver::onPlaybackAudioFrameBeforeMixing(uint64_t userID, NERtcAudioFrame* frame, channel_id_t cid) "onPlaybackAudioFrameBeforeMixing"[2/2] instead. In multi-channel scenarios, channelId is used to identify different channels.
     ...
     * @note 返回音频数据只读。
     * @deprecated 即将废弃，请改用接口 \ref nertc::INERtcAudioFrameObserver::onPlaybackAudioFrameBeforeMixing(uint64_t userID, NERtcAudioFrame* frame, channel_id_t cid) "onPlaybackAudioFrameBeforeMixing"[2/2]。在多房间场景下，此接口可通过 cid 识别不同房间。
     * @param userID 用户 ID。
     * @param frame  音频帧。
     * @endif
     */
     ...
     * @if Chinese
     * 获取指定远端用户混音前的辅流音频数据。
     * <br>成功注册音频观测器后，如果订阅了远端辅流音频（默认订阅）且远端用户开启辅流音频后，SDK 会在捕捉到混音前的辅流音频数据时，触发该回调，将辅流音频数据回调给用户。
     * @note 返回音频数据只读。
     * @param userID 用户ID。
     * @param frame  音频帧。
     * @param cid    房间 ID。在多房间场景下，cid 用于识别不同的房间。
...
struct NERtcVideoWatermarkTextConfig {
    ...
    NERtcVideoWatermarkTextConfig() : font_color(0xFFFFFFFF), font_size(15), wm_color(0x88888888), wm_alpha(1.0f), wm_width(0), wm_height(0), offset_x(0), offset_y(0) {
        memset(content, 0, sizeof(content));
        memset(font_path, 0, sizeof(font_path));
    }
};
...
struct NERtcVideoWatermarkTimestampConfig {
    ...
    NERtcVideoWatermarkTimestampConfig() : font_color(0xFFFFFFFF), font_size(15), wm_color(0x88888888), wm_alpha(1.0f), wm_width(0), wm_height(0), offset_x(0), offset_y(0) {
        memset(font_path, 0, sizeof(font_path));
    }
};
...
struct NERtcLastmileProbeConfig {
    ...
    NERtcLastmileProbeConfig()
        : probe_uplink(true), probe_downlink(true), expected_uplink_bitratebps(2000000), expected_downlink_bitratebps(2000000) {}
};
 ...
 * @note
 * You can change the subscription stautus. Local subscription status will not be affected if remote users resume sending the video stream; but local settings will resume to be default if remote users rejoin the room.
