...
public:
    ...
    */
     ...
     * @since V4.6.10
     * @param[in] uidArray 可订阅自己音频的用户uid 列表。
     ...
     * - Higher collection parameters means higher performance consumption, such as CPU and memory usage, especially when video pre-processing is enabled. 
     * @since V4.5.0
     * @param[in] type      Stream type. The default value is kNERTCVideoStreamMain. For more information, see #NERtcVideoStreamType.
     ...
     * - 设置更高的采集参数会导致更大的性能消耗，例如 CPU 和内存占用等，尤其是在开启视频前处理的场景下。
     * @since V4.5.0
     * @param[in] type      视频流通道类型，默认为主流kNERTCVideoStreamMain。 #NERtcVideoStreamType
     * @param[in] config    摄像头采集偏好配置。详细说明请参考 nertc::NERtcCameraCaptureConfig 。
     * @return {@code 0} 方法调用成功，其他调用失败
     * @endif
     */
     ...
     * 设置本地视频的编码属性。
     * - 可以在加入房间前或加入房间后调用此接口。
     * - `NERtcVideoConfig` 中的 `max_profile` 可以指定预设的 Profile 模式，但是预设模式往往无法满足实际场景需求，网易云信建议您通过 `width` 和 `height` 进行自定义设置。详细信息请参考[设置视频属性](https://doc.yunxin.163.com/docs/jcyOTA0ODM/zYwMTQyNzE)。
     * @note
    ...
    */
     ...
     * 本地的视频镜像模式仅影响本地用户所见，不影响远端用户所见。App 可以多次调用此方法更改镜像模式。
     * @param[in] mirror_mode   视频镜像模式。详细信息请参考 {@link NERtcVideoMirrorMode}。
     * @param[in] type          启用的视频流通道类型，默认为主流kNERTCVideoStreamMain。 #NERtcVideoStreamType
     * @return
     * - 0: 方法调用成功；
     * - 其他: 方法调用失败。
    * @endif
    ...
    */
     ...
     * @param[in] type      视频流通道类型。默认为 kNERTCVideoStreamMain。 详细信息请参考 #NERtcVideoStreamType Stream type。 
     * @param[in] mute
     * - true: 不发送本地视频流
     * - false: 发送本地视频流（默认）
     * @return
    ...
    */
     ...
     * - false：删除水印。
     * @param type   视频流类型。支持设置为主流或辅流。详细信息请参考 #NERtcVideoStreamType 。
     * @param config 画布水印设置。支持设置文字水印、图片水印和时间戳水印。详细信息请参考 \ref NERtcVideoWatermarkConfig。
     * @return
     * - 0: 方法调用成功；
     * - 其他: 方法调用失败。
     * @endif
    ...
    */
     ...
     * @since V4.6.0
     * @note
     * - 您可以通过 \ref nertc::IRtcEngineEx::onVirtualBackgroundSourceEnabled "onVirtualBackgroundSourceEnabled" 回调查看虚拟背景是否开启成功或出错原因。
